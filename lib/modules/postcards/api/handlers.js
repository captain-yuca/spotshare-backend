  ...
  return {
    ...
    getPostcards: async function (request, h) {
      ...
      if (visibility === 'shared') {
        ...
        if (query !== '') {
          postcards = await postcardService.searchPublicPostcards(options, query)
        } else {
          postcards = await postcardService.getPublicPostcards(options)
        }
      } else {
        if (query !== '') {
    ...
    collectPostcard: async function (request, h) {
      ...
      try {
        ...
        spot = {
          id: payload.id,
          name: payload.name,
          photoUrl: `https://picsum.photos/id/${faker.random.number(1, 200)}/500/500`,
     ...
     * PATCH /api/postcards
     */
    editPostcard: async function (request, h) {
      let user = (await request.auth.credentials)
      let id = encodeURIComponent(request.params.id)
      ...
      let postcard
      try {
        await postcardService.getPostcardById(id, user)
        postcard = await postcardService.editPostcard(id, payload)
      } catch (err) {
        if (err instanceof NotFoundError) {
          throw Boom.notFound('Postcard does not exist!')
        } else if (err.name === 'AuthorizationError') {
    ...
    /**
     * DELETE /api/postcards{id}/tags/{text}
     */
    removeTag: async function (request, h) {
      let user = (await request.auth.credentials)
      let postcardId = encodeURIComponent(request.params.id)
      let tagText = encodeURIComponent(request.params.text)
      let tagType = request.query.type
      const { postcardService } = request.services()
      let tags
      try {
        tags = await postcardService.removeTagFromPostcard(tagText, tagType, postcardId, user)
      } catch (err) {
        if (err instanceof NotFoundError) {
          throw Boom.notFound('Postcard does not exist!')
        } else if (err.name === 'AuthorizationError') {
