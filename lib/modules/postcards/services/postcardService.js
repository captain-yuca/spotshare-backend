...
class PostcardService extends Schmervice.Service {
  ...
  async getPostcardById (postcardId, user, options) {
    ...
    if (!this.canUserViewPostcard(postcard, user)) {
  ...
  async searchPublicPostcards (options, query) {
    const { postcardRepository } = this.server.services()
    let { page, limit, attrs } = options
    let result = await postcardRepository.searchPublicPostcards(page, limit, attrs, query)
    return result
  }

  async getPublicPostcards (options) {
    const { postcardRepository } = this.server.services()
    let { page, limit, attrs } = options
    let result = await postcardRepository.getPublicPostcards(page, limit, attrs)
    return result
  }
  ...
  canUserViewPostcard (postcard, user) {
    let tags = postcard.tags
    for (let i = 0; i < tags.length; i++) {
      if (
        tags[i].type === 'public' ||
        (tags[i].type === 'sharing' && tags[i].text === user.username)
      ) { return true }
    }
    if (postcard.userId === user.uid) { return true }
