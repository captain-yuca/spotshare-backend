...
class PostcardRepository extends Schmervice.Service {
  ...
  async findById (id) {
    ...
    let postcard = await Postcard.query()
      .findById(id)
      .throwIfNotFound()
    let tags = await this.getAllTagsFromId(id)
    postcard.tags = tags
    return postcard
  }
  ...
  async searchPublicPostcards (page, limit, attrs, query) {
    const { Postcard } = this.server.models()
    let result
    try {
      result = await Postcard
        ...
        .where('title', 'like', `%${query}%`)
        .andWhere(function () {
          this.whereExists(Postcard.relatedQuery('tags').where({ 'isPublicTag': true }))
        })
        .page(page, limit)
    } catch (err) {
      console.error(err)
